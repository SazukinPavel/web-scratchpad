...
export default function EditNote() {
  ...
  } = useQuery(OneNoteQuery, { variables: { id } });

  const { refetch: refetchNotes } = useQuery(GetNotesQuery);

  const [updateNoteMutation, { loading: isUpdateLoading, error: updateError }] =
  ...
  useEffect(() => {
    setOldNote({
      ...(getNoteData?.oneNote || {}),
      topic: getNoteData?.oneNote?.topic?.title,
    });
  }, [getNoteData]);
  ...
  return (
    <Container component="main" maxWidth="xs">
      <NoteForm
        ...
        btn={{ isLoading: isUpdateLoading, title: "Save" }}
      />
    </Container>
  );
}
